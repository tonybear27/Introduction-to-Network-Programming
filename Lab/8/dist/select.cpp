...
unsigned int last_assigned_ip = ADDRBASE;
int client_sockets[MAX_CLIENTS];

void initializeClientSocketsArray() {
    for (int i = 0; i < MAX_CLIENTS; i++) {
        client_sockets[i] = -1;  
    }
}

int addClientSocket(int clientSocket) {
    for (int i = 0; i < MAX_CLIENTS; i++) {
...
void handlePing(int tunFd, int clientSocket) {
...
int tunvpn_server(int port) {
    ...
    while(1) { 
        ...
        if (FD_ISSET(serverSocket, &readSet)) {
            ...
            if (addClientSocket(clientSocket) == -1) {
                perror("Maximum clients reached or error adding client");
                close(clientSocket);
                continue;
            }

            unsigned int clientIP = htonl(last_assigned_ip++);
            send(clientSocket, &clientIP, sizeof(clientIP), 0);
        }

        // Check each client socket for read activity
        for (int i = 0; i < MAX_CLIENTS; i++) {
            int clientSocket = client_sockets[i];
            if ((clientSocket > 0) && (FD_ISSET(clientSocket, &readSet) || FD_ISSET(tunFd, &readSet))) {
                handlePing(tunFd, clientSocket);
            }
        }
    }
