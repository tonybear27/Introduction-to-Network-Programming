...
struct client_mapping client_mappings[100];  
int num_clients = 0; 
...
void *handleClient(void *args) {
	...
	while(1) {
        ...
        unsigned int dst_ip = ip_header->ip_dst.s_addr;
		printf("%u.%u.%u.%u\n", NIPQUAD(dst_ip));
		if(nread > 0)
		...
		if(nnread > 0) {
			...
			for (int i = 0; i < num_clients; i++) {
            	if (client_mappings[i].ip == dest_ip) {
                	write(client_mappings[i].socket_fd, buffer, nnread);
                	break;
            	}
       	 	}
		}
...
int tunvpn_server(int port) {
    ...
    while(1) { 
		...
		client_mappings[num_clients].ip = clientIP;
		client_mappings[num_clients].socket_fd = clientSocket;
		num_clients++;
